// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from chess_arm_executor:action/ExecuteMove.idl
// generated code does not contain a copyright notice
#include "chess_arm_executor/action/detail/execute_move__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `src_square`
// Member `dst_square`
#include "rosidl_runtime_c/string_functions.h"

bool
chess_arm_executor__action__ExecuteMove_Goal__init(chess_arm_executor__action__ExecuteMove_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // mode
  // src_square
  if (!rosidl_runtime_c__String__init(&msg->src_square)) {
    chess_arm_executor__action__ExecuteMove_Goal__fini(msg);
    return false;
  }
  // dst_square
  if (!rosidl_runtime_c__String__init(&msg->dst_square)) {
    chess_arm_executor__action__ExecuteMove_Goal__fini(msg);
    return false;
  }
  // piece_type
  return true;
}

void
chess_arm_executor__action__ExecuteMove_Goal__fini(chess_arm_executor__action__ExecuteMove_Goal * msg)
{
  if (!msg) {
    return;
  }
  // mode
  // src_square
  rosidl_runtime_c__String__fini(&msg->src_square);
  // dst_square
  rosidl_runtime_c__String__fini(&msg->dst_square);
  // piece_type
}

bool
chess_arm_executor__action__ExecuteMove_Goal__are_equal(const chess_arm_executor__action__ExecuteMove_Goal * lhs, const chess_arm_executor__action__ExecuteMove_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // mode
  if (lhs->mode != rhs->mode) {
    return false;
  }
  // src_square
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->src_square), &(rhs->src_square)))
  {
    return false;
  }
  // dst_square
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->dst_square), &(rhs->dst_square)))
  {
    return false;
  }
  // piece_type
  if (lhs->piece_type != rhs->piece_type) {
    return false;
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_Goal__copy(
  const chess_arm_executor__action__ExecuteMove_Goal * input,
  chess_arm_executor__action__ExecuteMove_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // mode
  output->mode = input->mode;
  // src_square
  if (!rosidl_runtime_c__String__copy(
      &(input->src_square), &(output->src_square)))
  {
    return false;
  }
  // dst_square
  if (!rosidl_runtime_c__String__copy(
      &(input->dst_square), &(output->dst_square)))
  {
    return false;
  }
  // piece_type
  output->piece_type = input->piece_type;
  return true;
}

chess_arm_executor__action__ExecuteMove_Goal *
chess_arm_executor__action__ExecuteMove_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_Goal * msg = (chess_arm_executor__action__ExecuteMove_Goal *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(chess_arm_executor__action__ExecuteMove_Goal));
  bool success = chess_arm_executor__action__ExecuteMove_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
chess_arm_executor__action__ExecuteMove_Goal__destroy(chess_arm_executor__action__ExecuteMove_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    chess_arm_executor__action__ExecuteMove_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
chess_arm_executor__action__ExecuteMove_Goal__Sequence__init(chess_arm_executor__action__ExecuteMove_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_Goal * data = NULL;

  if (size) {
    data = (chess_arm_executor__action__ExecuteMove_Goal *)allocator.zero_allocate(size, sizeof(chess_arm_executor__action__ExecuteMove_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = chess_arm_executor__action__ExecuteMove_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        chess_arm_executor__action__ExecuteMove_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
chess_arm_executor__action__ExecuteMove_Goal__Sequence__fini(chess_arm_executor__action__ExecuteMove_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      chess_arm_executor__action__ExecuteMove_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

chess_arm_executor__action__ExecuteMove_Goal__Sequence *
chess_arm_executor__action__ExecuteMove_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_Goal__Sequence * array = (chess_arm_executor__action__ExecuteMove_Goal__Sequence *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = chess_arm_executor__action__ExecuteMove_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
chess_arm_executor__action__ExecuteMove_Goal__Sequence__destroy(chess_arm_executor__action__ExecuteMove_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    chess_arm_executor__action__ExecuteMove_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
chess_arm_executor__action__ExecuteMove_Goal__Sequence__are_equal(const chess_arm_executor__action__ExecuteMove_Goal__Sequence * lhs, const chess_arm_executor__action__ExecuteMove_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_Goal__Sequence__copy(
  const chess_arm_executor__action__ExecuteMove_Goal__Sequence * input,
  chess_arm_executor__action__ExecuteMove_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(chess_arm_executor__action__ExecuteMove_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    chess_arm_executor__action__ExecuteMove_Goal * data =
      (chess_arm_executor__action__ExecuteMove_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!chess_arm_executor__action__ExecuteMove_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          chess_arm_executor__action__ExecuteMove_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `message`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
chess_arm_executor__action__ExecuteMove_Result__init(chess_arm_executor__action__ExecuteMove_Result * msg)
{
  if (!msg) {
    return false;
  }
  // ok
  // message
  if (!rosidl_runtime_c__String__init(&msg->message)) {
    chess_arm_executor__action__ExecuteMove_Result__fini(msg);
    return false;
  }
  return true;
}

void
chess_arm_executor__action__ExecuteMove_Result__fini(chess_arm_executor__action__ExecuteMove_Result * msg)
{
  if (!msg) {
    return;
  }
  // ok
  // message
  rosidl_runtime_c__String__fini(&msg->message);
}

bool
chess_arm_executor__action__ExecuteMove_Result__are_equal(const chess_arm_executor__action__ExecuteMove_Result * lhs, const chess_arm_executor__action__ExecuteMove_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // ok
  if (lhs->ok != rhs->ok) {
    return false;
  }
  // message
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->message), &(rhs->message)))
  {
    return false;
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_Result__copy(
  const chess_arm_executor__action__ExecuteMove_Result * input,
  chess_arm_executor__action__ExecuteMove_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // ok
  output->ok = input->ok;
  // message
  if (!rosidl_runtime_c__String__copy(
      &(input->message), &(output->message)))
  {
    return false;
  }
  return true;
}

chess_arm_executor__action__ExecuteMove_Result *
chess_arm_executor__action__ExecuteMove_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_Result * msg = (chess_arm_executor__action__ExecuteMove_Result *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(chess_arm_executor__action__ExecuteMove_Result));
  bool success = chess_arm_executor__action__ExecuteMove_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
chess_arm_executor__action__ExecuteMove_Result__destroy(chess_arm_executor__action__ExecuteMove_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    chess_arm_executor__action__ExecuteMove_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
chess_arm_executor__action__ExecuteMove_Result__Sequence__init(chess_arm_executor__action__ExecuteMove_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_Result * data = NULL;

  if (size) {
    data = (chess_arm_executor__action__ExecuteMove_Result *)allocator.zero_allocate(size, sizeof(chess_arm_executor__action__ExecuteMove_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = chess_arm_executor__action__ExecuteMove_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        chess_arm_executor__action__ExecuteMove_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
chess_arm_executor__action__ExecuteMove_Result__Sequence__fini(chess_arm_executor__action__ExecuteMove_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      chess_arm_executor__action__ExecuteMove_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

chess_arm_executor__action__ExecuteMove_Result__Sequence *
chess_arm_executor__action__ExecuteMove_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_Result__Sequence * array = (chess_arm_executor__action__ExecuteMove_Result__Sequence *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = chess_arm_executor__action__ExecuteMove_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
chess_arm_executor__action__ExecuteMove_Result__Sequence__destroy(chess_arm_executor__action__ExecuteMove_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    chess_arm_executor__action__ExecuteMove_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
chess_arm_executor__action__ExecuteMove_Result__Sequence__are_equal(const chess_arm_executor__action__ExecuteMove_Result__Sequence * lhs, const chess_arm_executor__action__ExecuteMove_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_Result__Sequence__copy(
  const chess_arm_executor__action__ExecuteMove_Result__Sequence * input,
  chess_arm_executor__action__ExecuteMove_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(chess_arm_executor__action__ExecuteMove_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    chess_arm_executor__action__ExecuteMove_Result * data =
      (chess_arm_executor__action__ExecuteMove_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!chess_arm_executor__action__ExecuteMove_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          chess_arm_executor__action__ExecuteMove_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stage`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
chess_arm_executor__action__ExecuteMove_Feedback__init(chess_arm_executor__action__ExecuteMove_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // stage
  if (!rosidl_runtime_c__String__init(&msg->stage)) {
    chess_arm_executor__action__ExecuteMove_Feedback__fini(msg);
    return false;
  }
  return true;
}

void
chess_arm_executor__action__ExecuteMove_Feedback__fini(chess_arm_executor__action__ExecuteMove_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // stage
  rosidl_runtime_c__String__fini(&msg->stage);
}

bool
chess_arm_executor__action__ExecuteMove_Feedback__are_equal(const chess_arm_executor__action__ExecuteMove_Feedback * lhs, const chess_arm_executor__action__ExecuteMove_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // stage
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->stage), &(rhs->stage)))
  {
    return false;
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_Feedback__copy(
  const chess_arm_executor__action__ExecuteMove_Feedback * input,
  chess_arm_executor__action__ExecuteMove_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // stage
  if (!rosidl_runtime_c__String__copy(
      &(input->stage), &(output->stage)))
  {
    return false;
  }
  return true;
}

chess_arm_executor__action__ExecuteMove_Feedback *
chess_arm_executor__action__ExecuteMove_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_Feedback * msg = (chess_arm_executor__action__ExecuteMove_Feedback *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(chess_arm_executor__action__ExecuteMove_Feedback));
  bool success = chess_arm_executor__action__ExecuteMove_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
chess_arm_executor__action__ExecuteMove_Feedback__destroy(chess_arm_executor__action__ExecuteMove_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    chess_arm_executor__action__ExecuteMove_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
chess_arm_executor__action__ExecuteMove_Feedback__Sequence__init(chess_arm_executor__action__ExecuteMove_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_Feedback * data = NULL;

  if (size) {
    data = (chess_arm_executor__action__ExecuteMove_Feedback *)allocator.zero_allocate(size, sizeof(chess_arm_executor__action__ExecuteMove_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = chess_arm_executor__action__ExecuteMove_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        chess_arm_executor__action__ExecuteMove_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
chess_arm_executor__action__ExecuteMove_Feedback__Sequence__fini(chess_arm_executor__action__ExecuteMove_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      chess_arm_executor__action__ExecuteMove_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

chess_arm_executor__action__ExecuteMove_Feedback__Sequence *
chess_arm_executor__action__ExecuteMove_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_Feedback__Sequence * array = (chess_arm_executor__action__ExecuteMove_Feedback__Sequence *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = chess_arm_executor__action__ExecuteMove_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
chess_arm_executor__action__ExecuteMove_Feedback__Sequence__destroy(chess_arm_executor__action__ExecuteMove_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    chess_arm_executor__action__ExecuteMove_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
chess_arm_executor__action__ExecuteMove_Feedback__Sequence__are_equal(const chess_arm_executor__action__ExecuteMove_Feedback__Sequence * lhs, const chess_arm_executor__action__ExecuteMove_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_Feedback__Sequence__copy(
  const chess_arm_executor__action__ExecuteMove_Feedback__Sequence * input,
  chess_arm_executor__action__ExecuteMove_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(chess_arm_executor__action__ExecuteMove_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    chess_arm_executor__action__ExecuteMove_Feedback * data =
      (chess_arm_executor__action__ExecuteMove_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!chess_arm_executor__action__ExecuteMove_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          chess_arm_executor__action__ExecuteMove_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "chess_arm_executor/action/detail/execute_move__functions.h"

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Request__init(chess_arm_executor__action__ExecuteMove_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    chess_arm_executor__action__ExecuteMove_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!chess_arm_executor__action__ExecuteMove_Goal__init(&msg->goal)) {
    chess_arm_executor__action__ExecuteMove_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
chess_arm_executor__action__ExecuteMove_SendGoal_Request__fini(chess_arm_executor__action__ExecuteMove_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  chess_arm_executor__action__ExecuteMove_Goal__fini(&msg->goal);
}

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Request__are_equal(const chess_arm_executor__action__ExecuteMove_SendGoal_Request * lhs, const chess_arm_executor__action__ExecuteMove_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!chess_arm_executor__action__ExecuteMove_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Request__copy(
  const chess_arm_executor__action__ExecuteMove_SendGoal_Request * input,
  chess_arm_executor__action__ExecuteMove_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!chess_arm_executor__action__ExecuteMove_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

chess_arm_executor__action__ExecuteMove_SendGoal_Request *
chess_arm_executor__action__ExecuteMove_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_SendGoal_Request * msg = (chess_arm_executor__action__ExecuteMove_SendGoal_Request *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Request));
  bool success = chess_arm_executor__action__ExecuteMove_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
chess_arm_executor__action__ExecuteMove_SendGoal_Request__destroy(chess_arm_executor__action__ExecuteMove_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    chess_arm_executor__action__ExecuteMove_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence__init(chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_SendGoal_Request * data = NULL;

  if (size) {
    data = (chess_arm_executor__action__ExecuteMove_SendGoal_Request *)allocator.zero_allocate(size, sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = chess_arm_executor__action__ExecuteMove_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        chess_arm_executor__action__ExecuteMove_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence__fini(chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      chess_arm_executor__action__ExecuteMove_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence *
chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence * array = (chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence__destroy(chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence__are_equal(const chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence * lhs, const chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence__copy(
  const chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence * input,
  chess_arm_executor__action__ExecuteMove_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    chess_arm_executor__action__ExecuteMove_SendGoal_Request * data =
      (chess_arm_executor__action__ExecuteMove_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!chess_arm_executor__action__ExecuteMove_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          chess_arm_executor__action__ExecuteMove_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Response__init(chess_arm_executor__action__ExecuteMove_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    chess_arm_executor__action__ExecuteMove_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
chess_arm_executor__action__ExecuteMove_SendGoal_Response__fini(chess_arm_executor__action__ExecuteMove_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Response__are_equal(const chess_arm_executor__action__ExecuteMove_SendGoal_Response * lhs, const chess_arm_executor__action__ExecuteMove_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Response__copy(
  const chess_arm_executor__action__ExecuteMove_SendGoal_Response * input,
  chess_arm_executor__action__ExecuteMove_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

chess_arm_executor__action__ExecuteMove_SendGoal_Response *
chess_arm_executor__action__ExecuteMove_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_SendGoal_Response * msg = (chess_arm_executor__action__ExecuteMove_SendGoal_Response *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Response));
  bool success = chess_arm_executor__action__ExecuteMove_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
chess_arm_executor__action__ExecuteMove_SendGoal_Response__destroy(chess_arm_executor__action__ExecuteMove_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    chess_arm_executor__action__ExecuteMove_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence__init(chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_SendGoal_Response * data = NULL;

  if (size) {
    data = (chess_arm_executor__action__ExecuteMove_SendGoal_Response *)allocator.zero_allocate(size, sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = chess_arm_executor__action__ExecuteMove_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        chess_arm_executor__action__ExecuteMove_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence__fini(chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      chess_arm_executor__action__ExecuteMove_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence *
chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence * array = (chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence__destroy(chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence__are_equal(const chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence * lhs, const chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence__copy(
  const chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence * input,
  chess_arm_executor__action__ExecuteMove_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(chess_arm_executor__action__ExecuteMove_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    chess_arm_executor__action__ExecuteMove_SendGoal_Response * data =
      (chess_arm_executor__action__ExecuteMove_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!chess_arm_executor__action__ExecuteMove_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          chess_arm_executor__action__ExecuteMove_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
chess_arm_executor__action__ExecuteMove_GetResult_Request__init(chess_arm_executor__action__ExecuteMove_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    chess_arm_executor__action__ExecuteMove_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
chess_arm_executor__action__ExecuteMove_GetResult_Request__fini(chess_arm_executor__action__ExecuteMove_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
chess_arm_executor__action__ExecuteMove_GetResult_Request__are_equal(const chess_arm_executor__action__ExecuteMove_GetResult_Request * lhs, const chess_arm_executor__action__ExecuteMove_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_GetResult_Request__copy(
  const chess_arm_executor__action__ExecuteMove_GetResult_Request * input,
  chess_arm_executor__action__ExecuteMove_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

chess_arm_executor__action__ExecuteMove_GetResult_Request *
chess_arm_executor__action__ExecuteMove_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_GetResult_Request * msg = (chess_arm_executor__action__ExecuteMove_GetResult_Request *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Request));
  bool success = chess_arm_executor__action__ExecuteMove_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
chess_arm_executor__action__ExecuteMove_GetResult_Request__destroy(chess_arm_executor__action__ExecuteMove_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    chess_arm_executor__action__ExecuteMove_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence__init(chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_GetResult_Request * data = NULL;

  if (size) {
    data = (chess_arm_executor__action__ExecuteMove_GetResult_Request *)allocator.zero_allocate(size, sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = chess_arm_executor__action__ExecuteMove_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        chess_arm_executor__action__ExecuteMove_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence__fini(chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      chess_arm_executor__action__ExecuteMove_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence *
chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence * array = (chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence__destroy(chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence__are_equal(const chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence * lhs, const chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence__copy(
  const chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence * input,
  chess_arm_executor__action__ExecuteMove_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    chess_arm_executor__action__ExecuteMove_GetResult_Request * data =
      (chess_arm_executor__action__ExecuteMove_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!chess_arm_executor__action__ExecuteMove_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          chess_arm_executor__action__ExecuteMove_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "chess_arm_executor/action/detail/execute_move__functions.h"

bool
chess_arm_executor__action__ExecuteMove_GetResult_Response__init(chess_arm_executor__action__ExecuteMove_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!chess_arm_executor__action__ExecuteMove_Result__init(&msg->result)) {
    chess_arm_executor__action__ExecuteMove_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
chess_arm_executor__action__ExecuteMove_GetResult_Response__fini(chess_arm_executor__action__ExecuteMove_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  chess_arm_executor__action__ExecuteMove_Result__fini(&msg->result);
}

bool
chess_arm_executor__action__ExecuteMove_GetResult_Response__are_equal(const chess_arm_executor__action__ExecuteMove_GetResult_Response * lhs, const chess_arm_executor__action__ExecuteMove_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!chess_arm_executor__action__ExecuteMove_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_GetResult_Response__copy(
  const chess_arm_executor__action__ExecuteMove_GetResult_Response * input,
  chess_arm_executor__action__ExecuteMove_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!chess_arm_executor__action__ExecuteMove_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

chess_arm_executor__action__ExecuteMove_GetResult_Response *
chess_arm_executor__action__ExecuteMove_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_GetResult_Response * msg = (chess_arm_executor__action__ExecuteMove_GetResult_Response *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Response));
  bool success = chess_arm_executor__action__ExecuteMove_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
chess_arm_executor__action__ExecuteMove_GetResult_Response__destroy(chess_arm_executor__action__ExecuteMove_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    chess_arm_executor__action__ExecuteMove_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence__init(chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_GetResult_Response * data = NULL;

  if (size) {
    data = (chess_arm_executor__action__ExecuteMove_GetResult_Response *)allocator.zero_allocate(size, sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = chess_arm_executor__action__ExecuteMove_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        chess_arm_executor__action__ExecuteMove_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence__fini(chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      chess_arm_executor__action__ExecuteMove_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence *
chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence * array = (chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence__destroy(chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence__are_equal(const chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence * lhs, const chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence__copy(
  const chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence * input,
  chess_arm_executor__action__ExecuteMove_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(chess_arm_executor__action__ExecuteMove_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    chess_arm_executor__action__ExecuteMove_GetResult_Response * data =
      (chess_arm_executor__action__ExecuteMove_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!chess_arm_executor__action__ExecuteMove_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          chess_arm_executor__action__ExecuteMove_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "chess_arm_executor/action/detail/execute_move__functions.h"

bool
chess_arm_executor__action__ExecuteMove_FeedbackMessage__init(chess_arm_executor__action__ExecuteMove_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    chess_arm_executor__action__ExecuteMove_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!chess_arm_executor__action__ExecuteMove_Feedback__init(&msg->feedback)) {
    chess_arm_executor__action__ExecuteMove_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
chess_arm_executor__action__ExecuteMove_FeedbackMessage__fini(chess_arm_executor__action__ExecuteMove_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  chess_arm_executor__action__ExecuteMove_Feedback__fini(&msg->feedback);
}

bool
chess_arm_executor__action__ExecuteMove_FeedbackMessage__are_equal(const chess_arm_executor__action__ExecuteMove_FeedbackMessage * lhs, const chess_arm_executor__action__ExecuteMove_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!chess_arm_executor__action__ExecuteMove_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_FeedbackMessage__copy(
  const chess_arm_executor__action__ExecuteMove_FeedbackMessage * input,
  chess_arm_executor__action__ExecuteMove_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!chess_arm_executor__action__ExecuteMove_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

chess_arm_executor__action__ExecuteMove_FeedbackMessage *
chess_arm_executor__action__ExecuteMove_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_FeedbackMessage * msg = (chess_arm_executor__action__ExecuteMove_FeedbackMessage *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(chess_arm_executor__action__ExecuteMove_FeedbackMessage));
  bool success = chess_arm_executor__action__ExecuteMove_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
chess_arm_executor__action__ExecuteMove_FeedbackMessage__destroy(chess_arm_executor__action__ExecuteMove_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    chess_arm_executor__action__ExecuteMove_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence__init(chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_FeedbackMessage * data = NULL;

  if (size) {
    data = (chess_arm_executor__action__ExecuteMove_FeedbackMessage *)allocator.zero_allocate(size, sizeof(chess_arm_executor__action__ExecuteMove_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = chess_arm_executor__action__ExecuteMove_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        chess_arm_executor__action__ExecuteMove_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence__fini(chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      chess_arm_executor__action__ExecuteMove_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence *
chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence * array = (chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence *)allocator.allocate(sizeof(chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence__destroy(chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence__are_equal(const chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence * lhs, const chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence__copy(
  const chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence * input,
  chess_arm_executor__action__ExecuteMove_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(chess_arm_executor__action__ExecuteMove_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    chess_arm_executor__action__ExecuteMove_FeedbackMessage * data =
      (chess_arm_executor__action__ExecuteMove_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!chess_arm_executor__action__ExecuteMove_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          chess_arm_executor__action__ExecuteMove_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!chess_arm_executor__action__ExecuteMove_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
