...
public:
    ...
    tf_listener_(tf_buffer_)
  {
    ...
    approach_dz_    = this->declare_parameter<double>("approach_dz",         0.10); // 10 cm
    planner_id_     = this->declare_parameter<std::string>("planner_id",     "");   // e.g. "RRTConnectkConfigDefault"
...
private:
  ...
  void execute(std::shared_ptr<GoalHandle> gh)
  {
    ...
    // 1) Transform goal pose to base_frame if needed
    ...
    // Build approach pose (straight above by approach_dz_)
    geometry_msgs::msg::Pose approach = pick_in_base.pose;
    approach.position.z += approach_dz_;

    // 2) Approach from above
    if (!plan_and_execute(approach, "approach_pick", gh)) {
      fail(gh, result, "approach_pick");
      return;
    }

    // 3) Descend to exact pick pose
    if (!plan_and_execute(pick_in_base.pose, "grasp", gh)) {    // stage named "grasp" per your schema
      fail(gh, result, "grasp");
      return;
    }

    // Optional: lift back up (comment out if you don't want this)
    geometry_msgs::msg::Pose lift = approach; // same as approach height
    if (!plan_and_execute(lift, "lift", gh)) {
      fail(gh, result, "lift");
      return;
    }
  ...
  void fail(std::shared_ptr<GoalHandle> gh,
            std::shared_ptr<PickPlace::Result> res,
