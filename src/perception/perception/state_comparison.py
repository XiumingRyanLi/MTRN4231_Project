...
class ChessMoveDetector(Node):
    ...
    def occupancy_callback(self, msg):
        ...
        try:
            ...
            self.get_logger().error(
                f"Key error in occupancy callback: {e}. Keys: {list(occupancy_after.keys())[:5]}")
        except Exception as e:
            self.get_logger().error(
                f"Error in occupancy callback: {type(e).__name__}: {e}")
    ...
    def detect_move(self, occupancy_dict_before, occupancy_dict_after):
        ...
        occupancy_dict_before = {
            str(k): v for k, v in occupancy_dict_before.items()}
        occupancy_dict_after = {
            str(k): v for k, v in occupancy_dict_after.items()}
        ...
        self.get_logger().info(
            f"from squares: {from_squares}, to squares: {to_squares}")
        # Castling (2 from, 2 to - king and rook both move)
        if len(from_squares) == 2 and len(to_squares) == 2:
            castle_move = self.detect_castling(
                from_squares, to_squares, occupancy_dict_after)
            if castle_move:
                return castle_move

        # En passant (2 from, 1 to - attacking pawn moves, captured pawn disappears)
        elif len(from_squares) == 2 and len(to_squares) == 1:
            en_passant_move = self.detect_en_passant(
                from_squares, to_squares, occupancy_dict_before, occupancy_dict_after)
            if en_passant_move:
        ...
        for from_cell in from_cells:
            ...
            if abs(from_row - to_row) == 1 and abs(from_col - to_col) == 1:
                ...
                if occupancy_before[str(from_cell)] == occupancy_after[str(to_cell)]:
                    ...
                    captured_pawn = [
                        c for c in from_cells if c != moving_pawn][0]
                    break
